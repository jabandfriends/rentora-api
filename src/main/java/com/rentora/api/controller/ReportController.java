...
public class ReportController {
    ...
    public ResponseEntity<ApiResponse<PaginatedResponseWithMetadata<RoomReportDetailDTO, RoomReportMetaData>>> getRoomReport(
        ...
        Page<RoomReportDetailDTO> report = reportService.getRoomReport(apartmentId, pageable);
        RoomReportMetaData metadata = reportService.getRoomReportMetadata(apartmentId);

        return ResponseEntity.ok(
                ApiResponse.success(PaginatedResponseWithMetadata.of(report, page, metadata))
        );
    }
    ...
    public ResponseEntity<ApiResponse<PaginatedResponseWithMetadata<ReceiptReportDetailDTO, ReceiptReportMetaData>>> getAdhocInvoices(
            @PathVariable UUID apartmentId,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "createdAt") String sortBy,
        ...
        List<ReceiptReportDetailDTO> invoices = receiptReportService.getAdhocAndInvoices(apartmentId, status, pageable);Page<ReceiptReportDetailDTO> pageResult = new PageImpl<>(invoices, pageable, invoices.size());

        ReceiptReportMetaData metadata = receiptReportService.getReceiptReportMetadata(apartmentId);

        return ResponseEntity.ok(
                ApiResponse.success(PaginatedResponseWithMetadata.of(pageResult, page, metadata))
        );    }
